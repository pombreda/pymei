...
#
# This was "borrowed" from Terminator, which is a great application!
# See http://www.tenshu.net/terminator/ and https://launchpad.net/terminator/
# for more information about the project! :-)

from mei.version import APP_NAME, APP_VERSION
...
class PythonConsoleServer(SocketServer.BaseRequestHandler):
  env = None
  def setup(self):
    logging.debug('debugserver: connect from %s' % str(self.client_address))
...
def spawn(env):
  ...
  logging.debug("debugserver: listening on %s" % str(tcpserver.server_address))
