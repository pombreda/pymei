    ...
    def execute(self, selected):
        ...
        if os.path.isdir(selected):
            ...
            if files:
                ...
                self._app.windows.append(mplayer.Mplayer(self._app, selected, files))
        elif is_video.search(selected):
            self._app.windows.append(mplayer.Mplayer(self._app, os.path.dirname(selected), [os.path.basename(selected)]))
